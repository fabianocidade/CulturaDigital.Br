...
function uploadSuccess(file, serverData) {
	try {
		...
		if( serverData.indexOf('.mp4') != -1 ) {
			document.getElementById('mp4').value = serverData;
		}
		if( serverData.indexOf('.ogg') != -1 ) {
			document.getElementById('ogg').value = serverData;
		}
		if( serverData.indexOf('.webm') != -1 ) {
			document.getElementById('webm').value = serverData;
		}				
	} catch (ex) {
		console.log(ex);
	}
