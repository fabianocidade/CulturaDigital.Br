...
if ( !empty($_POST['attachToPost']) ) {
	
	$mp4 = $_POST['mp4'];
	$webm = $_POST['webm'];
	$ogg = $_POST['ogg'];
		
	$html = "[ctube mp4='$mp4' webm='$webm' ogg='$ogg']";
	
	if( !empty($html) || !empty($mp4) || !empty($ogg) || !empty($webm) ) {
		return my_media_send_to_editor($html);
	}
...
// Settings
	...
	$extension_whitelist = array("mp4", "ogg", "webm");	// Allowed file extensions
...
// Process the file
	...
	$file = wp_handle_upload($_FILES[$upload_name], $overrides);
